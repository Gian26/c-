...
/**************State****************/
/***********************************/
void State::addAgentData(AgentData _agentData){
    agentDataVector.push_back(_agentData);
}
...
vector<AgentData> State::getAgentDataV(){
  return agentDataVector;
}
...
void Swarm::moveUseState(GridWorld &_world,State _state){
 cout<< "STATE USE "<<endl;
  for (short agentIndex = 0; agentIndex < swarmSize; agentIndex++) {
    ...
    unsigned short _currentSpace = _state.getAgentDataV()[agentIndex].getPrevSpace();
    ...
    unsigned short _currentAction = _state.getAgentDataV()[agentIndex].getActionTaken();//obtener acción
